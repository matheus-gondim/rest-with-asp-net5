...
{
    [ApiVersion("1"), Route("api/[controller]/v{version:apiVersion}"), ApiController]
    public class AuthController : ControllerBase
    {
        ...
        public IActionResult Refresh([FromBody] TokenVO tokenVO)
        {
            ...
            var token = loginBusiness.ValidateCredentials(tokenVO);
            if (token == null) return BadRequest("Invalid client request");
            return Ok(token);
        }

        [HttpGet, Route("revoke"), Authorize("Bearer")]
